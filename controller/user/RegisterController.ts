...
import { GetAuthByEmail, RegisterAuthInfo } from "../../services/AuthServices";
import {
  PrepareEmail,
  PrepareMessage,
  SendMail,
...
export default async function RegisterController(req, res) {
  try {
    const data = req.body;
    const emailUser = await GetAuthByEmail(data.info.email);
    if (emailUser) {
      res.status(404).send("Email Address Already Taken");
    } else {
